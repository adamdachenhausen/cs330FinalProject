...
events, latencies and delays that take place in the very low level of memory movement.
We gathered the latency time, which is the time delay taking place in data transfer,reading and
writing at the lowest level of memory management.
...
Throughout the simulation the CPU was accessing the RAM and the Hard Drive to process
the data existing. We timed the transfer of data from both the Hard Drive and RAM.
...
were modified depending upon the event tanking place. This modification gave us the data for how the size
...
As the variable we had in the simulation were the number of Cores in the CPU,
